...
export const PersonalDetailsSchema = z.object({
  ...
  postcode: z
    .string()
    .min(5, { message: 'Postcode is required.' })
...
export const CollectionDetailsSchema = z.object({
  ...
  itemDescription: z.string().min(10, {
    message: 'Please describe the items (at least 10 characters).',
  }),
  estimatedVolume: z
    .string()
    .min(1, { message: 'Please estimate the volume.' }),
  accessRestrictions: z.string().optional(),
  preferredDate: z.date({ required_error: 'Preferred date is required.' }),
  preferredTime: z
    .string()
    .min(1, { message: 'Preferred time slot is required.' }),
  hasParking: z.enum(['yes', 'no', 'unsure'], {
    required_error: 'Please specify parking availability.',
  }),
});

export type CollectionDetailsFormData = z.infer<typeof CollectionDetailsSchema>;

export const BookingFormSchema = PersonalDetailsSchema.merge(
